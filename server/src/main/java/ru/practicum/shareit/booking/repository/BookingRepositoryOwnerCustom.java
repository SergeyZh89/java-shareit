...
public interface BookingRepositoryOwnerCustom {
    ...
    List<Booking> findAllByItem_OwnerAndEndIsBeforeOrderByIdDesc(long userid, LocalDateTime end, Pageable pageable);

    @Query(value = "SELECT * FROM bookings b " +
            "JOIN items i on i.id = b.item_id " +
            "WHERE status like ?2 AND i.owner_id = ?1", nativeQuery = true)
    List<Booking> findAllByOwner_IdAndByStatus(long ownerId, String state, Pageable pageable);
    ...
    @Query(value = "SELECT * from bookings " +
            ...
            nativeQuery = true)
    List<Booking> findBookingsByOwnerAndStatusFuture(long ownerId, LocalDateTime dateTime, Pageable pageable);
